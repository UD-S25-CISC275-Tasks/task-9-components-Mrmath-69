...
export function getPublishedQuestions(questions: Question[]): Question[] {
    return questions.filter((item) => item.published);
}
    ...
    return questions.map((item) =>
        item.id === targetId ?
            newQuestionType === "multiple_choice_question" ?
                { ...item, type: newQuestionType }
            :   { ...item, type: newQuestionType, options: [] }
        :   item,
    );
}
...
export function editOption(
    ...
    return questions.map((q) => {
        ...
        if (targetOptionIndex === -1) {
            // Append newOption to the end of the options array.
            newOptions = [...q.options, newOption];
        } else {
            // Replace the option at targetOptionIndex.
            newOptions = q.options.map((option, index) =>
